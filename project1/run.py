...
def cross_validate_model(model,x,y,k_fold=None,k_folds=5,random_seed=42,has_train_loss=False):
...
def evaluate_model(model,tx,y,k_fold,name="",has_train_loss=False):
    ...
    if has_train_loss:
        test_losses,train_losses = cross_validate_model(model,tx,y,k_fold=k_fold,has_train_loss=has_train_loss)
...
def main(config):
    ...
    preprocessing_methods = [lambda x: x,implementation.z_normalize,implementation.quantile_normalize,implementation.min_max_normalize]
    ...
    # Defining models:
    logistic_regression = implementation.Logistic_Regression_model(initial_w=np.zeros(tx.shape[1]), max_iters=2000, gamma=0.000003) 
    ...
    k_fold = implementation.K_fold(5) # Reusing same k_folds for performance and comparability reasons.
                        
    static_model_res = evaluate_model(static_model,tx,y,k_fold=k_fold,name="static model",has_train_loss=False)
    logistic_regression_res = evaluate_model(logistic_regression,tx,y,k_fold=k_fold,name="logistic regression",has_train_loss=True)
    least_squares_res = evaluate_model(least_squares,tx,y,k_fold=k_fold,name="least squares",has_train_loss=True)
    ...
    k_fold_normalized = implementation.K_fold(5) # Reusing same k_folds for performance and comparability reasons.
    
    logistic_regression = implementation.Logistic_Regression_model(initial_w=np.zeros(tx_z_normalized.shape[1]), max_iters=2000, gamma=0.000003) 

    static_model_res = evaluate_model(static_model,tx_z_normalized,y,k_fold=k_fold_normalized,name="static model")
    logistic_regression_res = evaluate_model(logistic_regression,tx_z_normalized,y,k_fold=k_fold_normalized,name="logistic regression",has_train_loss=True)
    least_squares_res = evaluate_model(least_squares,tx_z_normalized,y,k_fold=k_fold_normalized,name="least squares",has_train_loss=True)
    

    # With Interaction Matrix:
    print("Cross Validation With Interaction Matrix this time:")
    tx_interactions = implementation.build_interaction_tx(input_data, implementation.z_normalize)
    
    k_fold_interactions = implementation.K_fold(5) # Reusing same k_folds for performance and comparability reasons.
    
    logistic_regression = implementation.Logistic_Regression_model(initial_w=np.zeros(tx_interactions.shape[1]), max_iters=2000, gamma=0.000003) 

    static_model_res = evaluate_model(static_model,tx_interactions,y,k_fold=k_fold_interactions,name="static model")
    logistic_regression_res = evaluate_model(logistic_regression,tx_interactions,y,k_fold=k_fold_interactions,name="logistic regression",has_train_loss=True)
    least_squares_res = evaluate_model(least_squares,tx_interactions,y,k_fold=k_fold_interactions,name="least squares",has_train_loss=True)
...
#TODO
    ...
    best_model = implementation.Logistic_Regression_model( initial_w=np.zeros(tx_interactions.shape[1]), max_iters=8000, gamma=0.0000005)
    best_model_res = evaluate_model(best_model,tx_interactions,y,k_fold=k_fold_interactions,name="Best model LogReg")
    ...
    tx_test = implementation.build_interaction_tx(input_data_test, implementation.z_normalize)
